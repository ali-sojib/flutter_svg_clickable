...
class SvgParser {
  ...
  Color parseColor(String cStr) {
    ...
    if (cStr[0] == '#') {
      return Color(int.parse(cStr.substring(1), radix: 16)).withOpacity(1.0); // Hex to int: from https://stackoverflow.com/a/51290420/9452450
    } else if (cStr == 'none') {
  ...
  void loadFromString(String svgString) {
    ...
    doc.findElements("svg").map((e) => e.attributes).forEach((node) {
      var someH = node.firstWhere(
        (attr) => attr.name.local == "height",
        // orElse: () {
        // print('null');
        // return null;
        // }
      ...
      var someW = node.firstWhere(
        (attr) => attr.name.local == "width",
        // orElse: () {
        // print('null');
        // return null;
        // }
    ...
    doc.findAllElements("path").map((node) => node.attributes).forEach((attributes) {
      ...
      if (dPath != null) {
        ...
        if (style != null) {
          ...
          Match match = exp.firstMatch(style.value) as Match;
          if (match != null) {
          ...
          match = exp.firstMatch(style.value) as Match;
          if (match != null) {
            String? cStr = match.group(1);
